...
namespace FiftyOne.IpIntelligence.Engine.OnPremise.FlowElements
{
    ...
    public class IpiOnPremiseEngine : OnPremiseIpiEngineBase<IIpDataOnPremise>
    {
        ...
        /// will be read to
        /// <see cref="ProcessEngine(IFlowData, IIpDataOnPremise, int[])"/>
        /// so only the graphs those properties need are evaluated. Resolve
        ...
        {
            ProcessEngine(data, ipData, null);
        }
        ...
        /// always evaluated. That limit is accepted for performance.
        /// </remarks>
        /// <param name="data">
        /// The <see cref="IFlowData"/> instance containing data for the
        /// current request.
        ...
        /// order, so the position of each name is its index. A wrapper that
        /// supplies no list, such as a test double, leaves the map empty.
        /// </summary>
