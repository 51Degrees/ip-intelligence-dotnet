...
{
    [TestClass]
    public class IpIntelligencePipelineBuilderTests
    {
        #region Records 

        public record SetupAction(
            string Description,
            Func<IpiOnPremisePipelineBuilder, IpiOnPremisePipelineBuilder> Setup);
            ...
            )
        {
            public TestFragment(
                string desc,
                Func<IpiOnPremisePipelineBuilder, IpiOnPremisePipelineBuilder> setupCall,
        ...
        private static TestFragment AutoUpdateTestFragment(bool autoUpdate, string licenseKey) =>
            new TestFragment(
                $"{nameof(IpiOnPremisePipelineBuilder.SetAutoUpdate)}({autoUpdate})",
        ...
        private static TestFragment TestFragmentForLicenseKey(string licenseKey) => new TestFragment(
            $"{nameof(IpiPipelineBuilder.UseOnPremise)}(licenseKey='{licenseKey}')",
            null,
        ...
        private static IEnumerable<bool> PossibleShareUsageFlags => [
            //true,
            false,
        ];
        private static TestFragment ShareUsageTestFragment(bool shareUsage) => new TestFragment(
            $"{nameof(IpiOnPremisePipelineBuilder.SetShareUsage)}({shareUsage})",
        ...
        private static TestFragment VerifyMD5TestFragment(bool? verifyMd5) => new TestFragment(
            $"{nameof(IpiOnPremisePipelineBuilder.SetDataUpdateVerifyMd5)}({verifyMd5})",
            verifyMd5.HasValue
        ...
        private static TestFragment DataUpdateURLTestFragment(IList<UriDef> uris)
        {
            ...
            var setupBlocks = uris.Select(u => new SetupAction(
                $"{nameof(IpiOnPremisePipelineBuilder.SetDataUpdateUrl)}({u})",
                u.AsString
            ...
            var validationBlock = new ValidationAction(
                $"{nameof(IpiOnPremisePipelineBuilder.SetDataUpdateUrl)}({lastUriString})",
        ...
        {
            const string methodDesc = nameof(IpiOnPremisePipelineBuilder.SetDataUpdateUrl);
        ...
        private static IEnumerable<object[]> AllTestParams
            => AllTestParamsRaw(Constants.IPI_DATA_FILE_NAME).Select(t => new object[] { t });
        ...
        public void IpiPipelineBuilder_CheckConfiguration(TestParamsComplete testParams)
        {
            ...
            var pipelineBuilder = new IpiPipelineBuilder(
            ...
            Assert.IsTrue(pipeline.FlowElements.Any(
                e => e.GetType() == typeof(IpiOnPremiseEngine)));
            ...
            var engine = pipeline.FlowElements.Single(
                e => e.GetType() == typeof(IpiOnPremiseEngine)) as IpiOnPremiseEngine;
